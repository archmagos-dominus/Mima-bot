  ...
  bj_tables.forEach((item, i) => {
    ...
    if (item.started) {
      ...
      if (players_in_game) {
        ...
        embed.addFields([
        {name:`House hand: ${item.househand[0].value[1]}`,value:`${item.househand[0].rank}${item.househand[0].emoji}`}
        ]);
        ...
        item.players.forEach((player, j) => {
          ...
          if (item.player_state[j] === 0) {
            ...
            //add field to embed
            embed.addFields([
              {name:`${player.tag}'s hand: ${sum}`,value:`${cards}`}
            ]);
            ...
            if ((sum1 > 21) && (sum2 > 21)) {
              ...
              embed.addFields([
              {name:`LOST - ${player.tag}'s hand: ${sum})`,value:`${cards}`}
              ]);
              ...
              //add field to embed
              embed.addFields([{name:`WON - ${player.tag}'s hand: ${sum})`,value:`${cards}`}]);
              //give the turn to the next player
              temp = true; //helper var telling us when to stop
              //iterate through the players, starting from the next one
              for (var k = j+1; k < item.player_state.length; k++) {
              ...
              //add field to embed
              embed.addFields([{name:`${player.tag}'s hand: ${sum}`,value:`${cards}`}]);
            ...
            //add field to embed
            embed.addFields([{name:`LOST - ${player.tag}'s hand: ${sum}`,value:`${cards}`}]);
          ...
          //add field to embed
          embed.addFields([{name:`WON - ${player.tag}'s hand: ${sum}`,values:`${cards}`}]);
            ...
            //add field to embed
            embed.addFields([{name:`STAND - ${player.tag}'s hand: ${sum}`,value:`${cards}`}]);
          }
        });
      ...
      //there are no more players to make moves, give cards to the house and end the game
        ...
        //add field to embed
        embed.addFields([{name:`House hand: ${sum}`,value:`${house}`}]);
        //tally wins/loses
        if (bust) {
          //house lost
          item.players.forEach((player, j) => {
        ...
        item.players.forEach((player, k) => {
          ...
          if (item.player_state[k] === 2) {
            ...
            //add field to embed
            embed.addFields([{name:`DRAW - ${player.tag}'s hand: ${sum}`,value:`${cards}`}]);
          }
        });
      ...
      if (time - item.time > 10000) {
        ...
        item.deck.splice(0, 1);
        //add the house hand and it's value to be displayed
        embed.addFields([{name:`House hand: ${item.househand[0].value[1]}`,value:`${item.househand[0].rank}${item.househand[0].emoji}`}]);
        ...
        embed.addFields([{name:`Status:`,value:`Waiting for game start...`}]);
        //iterate through all the players
        var player_names = ``;
        item.players.forEach((player, j) => {
          player_names += `${player.tag}\n`
        });
        embed.addFields([{name:`Current players:`,value:`${player_names}`}]);
...
client.on("messageCreate", async (message) =>{
  ...
  if (config.month !== cmonth) {
    ...
    if (spevent[config.month].st === 1) {
      ...
      const eventend = new Discord.EmbedBuilder()
      ...
      var eventstart = new Discord.EmbedBuilder()
    ...
    //check if the message is sent in the botspam channel
          ...
          if (command === 'award') {
            ...
            const amount = amountStr === 'all' ? current
              ...
              embeds: [
                ...
                .setColor(0x00AE86)
                .addFields([
                {name: `I have gifted ${amount}💰 to ${target.tag}.`,value: `My current balance is ${currency.getBalance(client.user.id)}💰`}
                ])
              ]
            });
        	}
            ...
            return message.channel.send({
              embeds: [
                new Discord.EmbedBuilder()
                .setColor(0xFF0000)
                .addFields([{name:`I have taken ${amount}💰 from ${target.tag}`,value:`My current balance is ${currency.getBalance(client.user.id)}💰`}])
              ]
            });
        	}
          //award xp
      ...
      if (running) {
        ...
        if (command === "help") {
          ...
          var help = new Discord.EmbedBuilder()
        ...
        //buying roles from the shop
          ...
          //check the valitidy of the item
            ...
            //check if the user already owns the role
              ...
              {embeds:[new Discord.EmbedBuilder()
              .setColor(0x00AE86)
              .addFields([{name:`You've bought **${item.name}**`,value:`Your current balance is: ${currency.getBalance(message.author.id)}`}])]}
            );
      		} catch (e) {
        ...
        //displaying the shop
          ...
          //create the embed
          var shop = new Discord.EmbedBuilder()
          ...
          items.forEach(item => {
            ...
            try {
        			role.name;
              shop.addFields([{name:`"${item.name}":  ${item.cost} 💰`,value:`${item.desc}`}]);
              empty = false;
        		} catch (e){
              true == true
            }
          ...
          if (empty) shop.addFields([{name:`"Purple Marisa": 100000 💰`,value:`Gives "Purple Marisa" role and access to unique Mima-bot commands`}])
        ...
        //check if bet flipping
        	...
        	const cmds = commandArgs.split(' ');
      		...
      		//tally the win/loss
       		...
       		//make embed results
       		var flip = new Discord.EmbedBuilder()
            ...
            flip.setColor(0xFF0000);
      			flip.addFields([{name:`The coin flipped, and it was ${eside}`,value:'\u200B'}]);
      			if (msgbet) {
      				flip.addFields([{name:`You bet: ${bet}`, value:`You got rekt, enjoy being homeless`}]);
      			} else {
      				flip.addFields([{name:`You bet: ${bet}`, value:`And you lost lmao, try again`}]);
      			}
      		} else if (win === 1) {
      			//you win embed result
            flip.setColor(0x00AE86);
      			flip.addFields([{name:'\u200B', value:`The coin flipped, and it was ${eside}`}]);
      			if (msgbet) {
      				flip.addFields([{name:`You bet: ${bet}`, value:`And you won ${bet*2}! Wow, hax!`}]);
      			} else {
      				flip.addFields([{name:`You bet: ${bet}`, value:`Yay, you won ${bet*2}! Spare change...`}]);
      			}
      		}
      	...
      	//check if dice rolling
      		...
      		//inint embed results
      		var roll = new Discord.EmbedBuilder()
      			.setTitle(`Bet roll result for ${message.author.username}`)
            .setThumbnail(message.author.displayAvatarURL());
      		roll.addFields([{name:`The dice rolled, and it was:`, value:'\u200B'}]);
      		roll.addFields([{name:`${x}   ${y}`, value:'\u200B'}]);
      		roll.addFields([{name:`${xface} ${yface}`, value:'\u200B'}]);
      		//tally the win/loss
      		if (x === 6 && y === 6){
      			//win on double
            roll.setColor(0x00AE86);
      			...
      			if (msgbet) {
      				roll.addFields([{name:`You bet: ${bet}`, value:`And you won ${bet*8}! JACKPOT!`}]);
      			} else {
      				roll.addFields([{name:`You bet: ${bet}`, value:`You won ${bet*8}! Should have went all in...`}]);
      			}
      		}
      		else if (x+y > 7) {
      			//win on sum
            roll.setColor(0x00AE86);
      			...
      			if (msgbet) {
      				roll.addFields([{name:`You bet: ${bet}`, value:`And you won ${bet*2}! Wow, hax!`}]);
      			} else {
      				roll.addFields([{name:`You bet: ${bet}`, value:`Yay, you won ${bet*2}! Spare change...`}]);
      			}
      		} else {
      			//lose
            roll.setColor(0xFF0000);
      			...
      			//you lose embed result
      			if (msgbet) {
      				roll.addFields([{name:`You bet: ${bet}`, value:`You got rekt, enjoy being homeless`}]);
      			} else {
      				roll.addFields([{name:`You bet: ${bet}`, value:`And you lost lmao, try again`}]);
      			}
      		}
      		//display embed results
      	...
      	//check if turning wheel of fortune
        	...
        	const cmds = commandArgs.split(' ');
      		...
      		//build the embed
      		var wheelem = new Discord.EmbedBuilder()
      		...
      		wheelem.addFields([{name:`The wheel spun, and it landed on ${config.slice[x]}`, value:'\u200B'}]);
      		wheelem.setImage(config.wheelimg[x]);
      		wheelem.addFields([{name:`You bet: ${bet}`, value:`You won ${win}!`}]);
      		//display embed results
      	...
      	//check if playing rps
      		...
      		//make embed results
      		var rps = new Discord.EmbedBuilder()
      			.setTitle(`Rock/Paper/Scissors result for ${message.author.username}`)
      			.setThumbnail(message.author.displayAvatarURL());
      		if (win === 0) {
      			//draw embed result
      			rps.addFields([{name:`We both chose ${config.rps[x]}`, value:'\u200B'}]);
      			rps.addFields([{name:`It's a draw!`, value:`\u200B`}]);
      		} else if (win === 1) {
      			//you win embed result
            rps.setColor(0x00AE86);
      			rps.addFields([{name:'\u200B', value:`You chose ${config.rps[ch]} and I chose ${config.rps[x]}`}]);
      			if (msgbet) {
      				rps.addFields([{name:`You bet: ${bet}`, value:`And you won ${bet*2}! Wow, hax!`}]);
      			} else {
      				rps.addFields([{name:`You bet: ${bet}`, value:`Yay, you won ${bet*2}! Spare change...`}]);
      			}
      		} else if (win === -1){
      			//you lose embed result
            rps.setColor(0xFF0000);
      			rps.addFields([{name:'\u200B', value:`You chose ${config.rps[ch]} and I chose ${config.rps[x]}`}]);
      			if (msgbet) {
      				rps.addFields([{name:`You bet: ${bet}`, value:`And you lost everything`}]);
      			} else {
      				rps.addFields([{name:`You bet: ${bet}`, value:`And you lost ...`}]);
      			}
      		}
      		//display embed results
      	...
      	//check if playing the slot machine
      		...
      		//make embed results
      		var slots = new Discord.EmbedBuilder()
      		...
      		if (win === 7) {
      			...
      			slots.addFields([{name:`You got all the Mimas! WOW! EXTRA JACKPOT!`, value:'\u200B'}]);
      			slots.addFields([{name:`You win ${bet*100}`, value:`\u200B`}]);
      		} else if (win === 6) {
      			//you win small ajckpot embed result
      			slots.addFields([{name:`You got 4 of a kind! JACKPOT!`, value:'\u200B'}]);
      			slots.addFields([{name:`You win ${bet*25}`, value:`\u200B`}]);
      		} else if (win === 5) {
      			//you win on two pairs, one of them mima
      			slots.addFields([{name:`You got 2 pairs! And one of them is me!`, value:'\u200B'}]);
      			slots.addFields([{name:`You win ${bet*8}`, value:`\u200B`}]);
      		} else if (win === 4) {
      			//you win on triple
      			slots.addFields([{name:`You got a triple!`, value:'\u200B'}]);
      			slots.addFields([{name:`You win ${bet*5}`, value:`\u200B`}]);
      		} else if (win === 3) {
      			//you win on mima pair
      			slots.addFields([{name:`You got a pair of Mimas!`, value:'\u200B'}]);
      			slots.addFields([{name:`You win ${bet*2}`, value:`\u200B`}]);
      		} else if (win === 2) {
      			//you win on two pairs
      			slots.addFields([{name:`You got two pairs`, value:'\u200B'}]);
      			slots.addFields([{name:`You win ${bet*2}`, value:`\u200B`}]);
      		} else if (win === 1){
      			//you win  on a mima
      			slots.addFields([{name:`You found me! I'll give you some change lol!`, value:'\u200B'}]);
      			slots.addFields([{name:`You win ${Math.floor(bet*0.5)}`, value:`\u200B`}]);
      		} else {
      			//you lose embed result
            slots.setColor(0xFF0000);
      			slots.addFields([{name:`Seems like you didn't get anything good...`, value:`\u200B`}]);
      			if (msgbet) {
      				slots.addFields([{name:`You bet: ${bet}`, value:`And you lost everything`}]);
      			} else {
      				slots.addFields([{name:`You bet: ${bet}`, value:`And you lost it`}]);
      			}
      		}
      		//display embed results
        ...
        //check if user is starting a blackjack game
          ...
          embed.addFields([{name:`Status:`,value:`Waiting for game start...`}]);
          embed.addFields([{name:`Current players:`,value:`${message.author.tag}`}]);
        ...
        //jail command
        	...
        	if (message.mentions.users.first()) {
          	...
          	target = message.mentions.users.first();
      			...
      			tag = target.tag;
      			//get the avatar
                let avt = await Canvas.loadImage("https://cdn.discordapp.com/avatars/" + target.id + "/" > + target.avatar + ".png");
      		...
      		const attachment = new Discord.AttachmentBuilder(canvas.toBuffer(), {name:'welcome-image.png'});
            var embed = new Discord.EmbedBuilder()
      			.setTitle(`${tag} was put behind bars for being too horni!`)
        		.setImage(`attachment://welcome-image.png`);
        ...
        //check own or another user's xp
          ...
          return message.channel.send({embeds: [
            new Discord.EmbedBuilder()
              .addFields([{name:`${targetUser.tag} currently has ${target.xp} points, and is level ${target.level}!\n`,
                value:`Next level at ${Math.pow(5*(target.level+1),2)} points.`}])
          ]});
        }
        ...
        //check user waifu informations
          ...
          const embed = new Discord.EmbedBuilder()
            ...
            .setColor(0x00AE86)
            .addFields([{name:'Owned by: ', value:owner}])
            .addFields([{name:'Current value: ', value:`${targetWaifu?.value || config.WAIFU_DEFAULT}`}])
            .addFields([{name:'Gifts:', value:gifts}])
            .addFields([{name:'Waifus:', value:waifus}]);
          ...
          return message.channel.send({embeds:[
            new Discord.EmbedBuilder()
              .setColor(0xFF000)
              .addFields([{name:`It didn't work out between ${message.author} and ${target.tag}. They are now divorced.`, value:`They each got ${value} back.`}])
          ]});
        }
      	//display giftshop
            ...
            embeds:[
              new Discord.EmbedBuilder()
      			   ...
      			   .setColor(0x00AE86)
      			   .setThumbnail(message.author.displayAvatarURL())
      			   .addFields([{name:'ID. Name: Price', value:giftls}])
        ...
        if (command === 'eventstats') {
          ...
          return message.channel.send({
            embeds:[
              ...
              .setColor(0x00AE86)
              .addFields([{name:`Current points: `, value:`${target.points}`}])
              .addFields(
                [{name:`${spevent[config.month].itemicon}${spevent[config.month].itemsname}: `,
                value:`${target.items}`}]
              )
            ]
          });
        ...
        if (command === 'eventinfo') {
          //build embed
          var eventinfo = new Discord.EmbedBuilder()
            .setTitle(`Event info for ${spevent[config.month].name}`)
            .setColor(0x00AE86);
          //fetch points
          eventinfo.addFields([{name:`Event description: `, value:spevent[config.month].description}]);
          //display response
        ...
        //fortune telling (flavour """inspired""" from Stardew Valley)
          ...
          while (z === x || z === y) {
            z = Math.floor(Math.random()*config.tarotcard.length);
      		...
      		var cards = new Discord.EmbedBuilder()
      			.setTitle("Your tarot card reading")
      			.setColor(0x00AE86)
      			.setThumbnail(message.author.displayAvatarURL())
        		.setImage(`attachment://welcome-image.png`);
      		cards.addFields([{name:`The cards reflecting your destiny are:`, value:`\u200B`}]);
      		cards.addFields([{name:`Past:`, value:`${config.tarotdesc[x]}`}]);
      		cards.addFields([{name:`Present:`, value:`${config.tarotdesc[y]}`}]);
      		cards.addFields([{name:`Future:`, value:`${config.tarotdesc[z]}`}]);
      		//post the embed to the channel
    ...
    if (running) {
      ...
      //check for the ping command
    	if (mes === `ping` || mes === `pong`) {
    		...
    		ping.addFields([{name:`Latency:`, value:`${Date.now() - message.createdTimestamp}ms.`}]);
    		ping.addFields([{name:`API Latency:`, value:`${Math.round(client.ws.ping)}ms`}]);
