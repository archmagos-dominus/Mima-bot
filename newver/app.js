//add slots?
//add the abbility to bet on a coinflip
//rework the main help file
...
client.on('message', async message => {
	...
	currency.add(message.author.id, moni);
  ...
  return msg.channel.send(help);
	...
	client.on('message', msg => {
	 ...
	 var help = new Discord.MessageEmbed()
			...
			.setDescription("List of commands availible")
			.setColor(0x00AE86);
	 help.addField(`${PREFIX}help cash`, `Shows this message`);
	 help.addField(`${PREFIX}balance/${prefix}cash [@username]`, `Shows your (or [username]'s currency balance')`);
	 help.addField(`${PREFIX}leaderboard/${PREFIX}lb`, `Displays the currency leaderboard`);
	 help.addField(`${PREFIX}inventory/${prefix}inv`, `Displays the inventory`);
	 help.addField(`${PREFIX}give/${prefix}transfer [amount] [username]`, `Give [username] [amount] of coins`);
	 help.addField(`${PREFIX}shop`, `Displays the shop`);
	 help.addField(`${PREFIX}buy [name]`, `Buys the [name] item from the shop`);
	 help.addField(`${PREFIX}shop`, `Displays the shop`);
...
client.on('message', msg => {
 ...
 msg.channel.send(`Eww, no thanks!<:mimadonut:856559759888351262>`);
 }
 });
...
client.on("message", message => {
  ...
  if (message.guild) {
    ...
    const curLevel = Math.floor(xpmulti * Math.sqrt(client.points.get(key, "points")));
